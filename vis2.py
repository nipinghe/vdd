...
_FEAT_FORMAT = 'feat_image%d_chan%d'
...
def _save_annotation(image,save_dir,filename):
...
def _process_batch(sess, original_images, semantic_predictions, image_names,
                   ...
                   raw_save_dir, train_id_to_eval_id=None,input_feature,middle_feature,aspp):
  ...
  (original_images,
   ...
   image_names,
   image_heights,
   image_widths,input_feature,middle_feature,aspp[0],aspp[1],aspp[2],aspp[3]) = sess.run([original_images, semantic_predictions,
                             image_names, image_heights, image_widths,input_feature,middle_feature,aspp[0],aspp[1],aspp[2],aspp[3]])
  ...
  for i in range(num_image):
    ...
    for j in range(num_channel):
        _save_annotation(input_feat[:,:,j],save_dir+'_input',_FEAT_FORMAT % (image_id_offset + i,j))
    num_channel = middle_feat[-1]
    for j in range(num_channel):
        _save_annotation(middle_feat[:,:,j],save_dir+'_middle',_FEAT_FORMAT % (image_id_offset + i,j))
    num_channel = aspp0_feat[-1]
    for j in range(num_channel):
        _save_annotation(aspp0_feat[:,:,j],save_dir+'_aspp0',_FEAT_FORMAT % (image_id_offset + i,j))
    num_channel = aspp1_feat[-1]
    for j in range(num_channel):
        _save_annotation(aspp1_feat[:,:,j],save_dir+'_aspp1',_FEAT_FORMAT % (image_id_offset + i,j))
    num_channel = aspp2_feat[-1]
    for j in range(num_channel):
        _save_annotation(aspp2_feat[:,:,j],save_dir+'_aspp2',_FEAT_FORMAT % (image_id_offset + i,j))
    num_channel = aspp3_feat[-1]
    for j in range(num_channel):
        _save_annotation(aspp3_feat[:,:,j],save_dir+'_aspp3',_FEAT_FORMAT % (image_id_offset + i,j))
    # Save image.
...
def main(unused_argv):
  ...
  with g.as_default():
	...
	input_feature = graph.get_tensor_by_name('xception_65/entry_flow/conv1_2/Relu:0')
	middle_feature = graph.get_tensor_by_name('xception_65/middle_flow/block1/unit_3/xception_module/Relu:0')
	for i in range(	4):
		aspp[i] = graph.get_tensor_by_name('aspp'+str(i)+('' if i==0 else '_pointwise') + '/Relu:0')
        image_id_offset = 0
        for batch in range(num_batches):
          ...
          _process_batch(sess=sess,
			 ...
			 input_feature,middle_feature,aspp)
